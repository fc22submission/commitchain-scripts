import {
  ETHEREUM_NETWORK,
  INFURA_PROJECT_ID,
  optimismStateChainContractAddress,
} from "./config";
import { providers, ethers, BigNumber, Contract } from "ethers";
import { ovmStateChainAbi } from "./optimismAbi";
import { OptimismTransactionBatch } from "./optimismOrderingScript";
...
async function processOVMStateChain(
...
(async () => {
  ...
  const topics = transactionContract.filters.StateBatchAppended();
  ...
  while (true) {
    ...
    // Process the batches...
    const { txNo, gasUsed, batchIndex } = await processOVMStateChain(
      events,
      transactionContract,
      provider
    );
